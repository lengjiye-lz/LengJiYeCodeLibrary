        ...
        public void checkServerTrusted(X509Certificate[] chain, String authType) throws CertificateException {
            try {
                ...
                for (X509Certificate cert : chain) {
                    LogTool.i("Longer==========>", "key=" + serverCert.getPublicKey());
                    LogTool.i("Longer==========>", "key=" + cert.getPublicKey());
                    cert.checkValidity();// 验证证书的合法性
                    cert.verify(serverCert.getPublicKey());// 验证服务端公钥和客户端是否一致，不一致请求会中断
                }
            } catch (Exception e) {
